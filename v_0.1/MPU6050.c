...
void set_LPF(u8 peso){
	if(peso>6)return;
	peso = peso|0x08;
	MPU6050_WriteBits(MPU6050_DEFAULT_ADDRESS, MPU9250_RA_ACCEL_CONFIG2,0,4,peso);
}
...
uint8_t  get_LPF(){
	uint8_t temp;
	MPU6050_ReadBits(MPU6050_DEFAULT_ADDRESS, MPU9250_RA_ACCEL_CONFIG2, 0,3, &temp);
	return temp;
}
 ...
 * @return None
 */
void get_xy_f(float *xtetaf, float *ytetaf,float P,float T){
	float acx, acy, acz,gyx, gyy, gyz,teta_acelx, teta_acely, wxgy, wygy;
	s16 bytes[6];

	MPU6050_GetRawAccelGyro(&bytes);
	//acx = bytes[0]/A_R;
	acy = bytes[1]/A_R;
	acz = bytes[2]/A_R;
	...
	//teta_acelx = powf((teste),2);


	// Calcula o angulo de X com acelerometro em DG.
	teta_acelx = atan(acx / A_R / sqrt(pow((acy/A_R),2) + pow((acz/A_R),2)))*RAD_TO_DEG;
